...
## Telecom 通讯迁移补充（本轮）
- 目标切换为“电信通讯完整移植并准备联调”，且不改 `Beacon*` 代码路径。
- 新增 `common` 运行时：
  - `common/src/main/java/cn/hydcraft/hydronyasama/telecom/runtime/TelecomCommRuntime.java`
- 实现范围（对齐 `.reference/NyaSamaTelecom`）：
  1. 组件类型：`SignalBox/SignalBoxSender/SignalBoxGetter/TriStateSignalBox/RSLatch/Timer/Delayer/WirelessRx/WirelessTx`。
  2. 连线语义：`sender/target/transceiver` 三类链路。
  3. 逻辑语义：反相器、TriState 触发、RS 锁存、Timer/Delayer tick 状态机、外部总线输入、无线收发状态注入与读取。
  4. 联调能力：`snapshots()` 输出各组件输入/输出/启用态与连线态，便于客户端和服务端联调对齐。
- 构建验证：
...
### 验证
- `./gradlew.bat :fabric-1.16.5:build -x test` 通过。
## 1.16.5 OBJ 灰度扩容（调试用 5 个 + 3D 物品）
- 用户需求：增加几个“灰度 OBJ 模型物品”方便调试。
...
### 实施
1. 继续启用 1.16.5 OBJ bootstrap：
- `fabric-1.16.5/src/main/java/cn/hydcraft/hydronyasama/fabric/BeaconProviderFabricClient.java`

2. Handler 白名单扩大到 5 个 OBJ：
- `fabric-1.16.5/src/main/java/cn/hydcraft/hydronyasama/objrender/fabric/v116/ObjModelResourceHandler116.java`
- `CANARY_OBJ_PATHS` 从单个 `ad_board_base.obj` 扩到 5 个 `*_base.obj`。

3. 恢复 5 个 Fabric block OBJ 覆盖模型：
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/block/ad_board.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/block/cuball_lamp.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/block/led_plate.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/block/fluorescent_light.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/block/holo_jet_rev.json`

4. 新增 5 个调试用 3D 物品模型（parent 指向 block）：
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/item/ad_board.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/item/cuball_lamp.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/item/led_plate.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/item/fluorescent_light.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/item/holo_jet_rev.json`

### 验证
- `./gradlew.bat :fabric-1.16.5:build -x test` 通过。
## 1.16.5 OBJ 空白根因修复（Gson deepCopy IllegalAccessError）
- 用户日志明确报错：`IllegalAccessError: tried to access method com.google.gson.JsonObject.deepCopy()`。
- 根因：1.16.5 运行时 Gson 版本与编译期行为差异，`deepCopy()` 在该环境下不可直接访问。
- 修复文件：
  - `fabric-1.16.5/src/main/java/cn/hydcraft/hydronyasama/objrender/fabric/v116/ObjModelResourceHandler116.java`
- 修复内容：
  - 将所有 `JsonObject.deepCopy()` / `JsonElement.deepCopy()` 改为“`toString + GsonHelper.parse(StringReader)`”兼容拷贝；
  - 覆盖场景：
    - station_lamp 分支复制
    - `loadExtraLayer` 复制
    - `mergeParentChain` 合并复制
- 验证：`./gradlew.bat :fabric-1.16.5:build -x test` 通过。
## 1.16.5 再次修复（反复重载 + 方格字回归）
- 用户日志新根因：
  1) `copyJsonElement` 把 primitive 当 object 解析，抛 `JsonSyntaxException`；
  2) canary 物品模型使用 `parent -> block/xxx`，触发 `BlockModel parent has to be a block model`。

### 修复
1. JSON 复制兼容修复：
- `fabric-1.16.5/.../ObjModelResourceHandler116.java`
- `copyJsonObject` / `copyJsonElement` 改为 `JsonParser.parse(source.toString())`，不再强制按 object 解析 primitive。

2. canary 物品模型改为直接 `forge:obj`（不走 parent 链）：
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/item/ad_board.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/item/cuball_lamp.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/item/led_plate.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/item/fluorescent_light.json`
...
### 实施
1. 扩大 OBJ handler 白名单：
- `fabric-1.16.5/.../ObjModelResourceHandler116.java`
- 追加上述 6 个 `models/blocks/*_base.obj` 路径。

2. 恢复对应 block/item OBJ 覆盖模型（调试用）：
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/block/*.json`
- `fabric-1.16.5/src/main/resources/assets/hydronyasama/models/item/*.json`
- 每个均采用 `loader=forge:obj` 直接加载，不走 `parent`。

### 验证
- `./gradlew.bat :fabric-1.16.5:build -x test` 通过。
## 1.16.5 全量 OBJ 联调开启 + 周报补充
- 用户确认稳定后要求“全部 OBJ + 站台灯一次联调”。
- 本轮改动：
  - `ObjModelResourceHandler116` 从 canary 白名单切换为全量 `hydronyasama:models/blocks/*.obj`。
  - 将 `fabric-1.20.1` 的 21 个 block OBJ 覆盖模型同步到 `fabric-1.16.5`。
  - `fabric-1.16.5` item 侧同步同名 `forge:obj` 模型用于联调观察。
- 构建：`./gradlew.bat :fabric-1.16.5:build -x test` 通过。
- 周报：已新增 `.agents/周报_2026-02-16.md`。
## Telecom 接线层移植（收口）
- 新增 `common` 服务：`telecom/runtime/TelecomCommService`。
- 完成 6 目标（Fabric/Forge * 1.16.5/1.18.2/1.20.1）工具接线：
  - `ConnectorItem` 写入 sender/target/transceiver 连线。
  - `DevEditorItem` 写入 mode/inverter 到运行时。
  - `NgTabletItem` 输出 endpoint 快照到 `telecom_tablet_scan_state`。
- 依赖边界：未修改 BeaconNetwork 路径。

## 构建验证
- Fabric 三版本 build 全通过（-x test）。
- Forge 三版本 build 全通过（-x test）。
## Telecom 运行时 Tick 接入
- 为避免“仅点击工具才推进状态”，已在 6 个入口注入服务器 Tick：
  - Fabric 1.16.5/1.18.2/1.20.1：`BeaconProviderFabric`
  - Forge 1.16.5/1.18.2/1.20.1：`BeaconProviderForge`
- 接入内容：`TelecomCommService.tick()` + 服务停止 `reset()`。

## 二次全量构建
- 六目标（Fabric/Forge * 三版本）再次 build 全通过（-x test）。
## Telecom 交互反馈事件补全（六端统一）
- 统一改造 Connector/DevEditor/NgTablet（Fabric/Forge * 1.16.5/1.18.2/1.20.1）：
  - Connector：起点选中/取消/连接成功/断开/不兼容 全部有可见提示。
  - DevEditor：模式切换与反相器开关有可见提示。
  - NgTablet：扫描结果状态直接提示并写入 NBT。
- TelecomCommService 增加连接结果返回：`CONNECTED / DISCONNECTED / INCOMPATIBLE`。
- 兼容处理：1.20.1 使用 `Component.literal`；1.16.5/1.18.2 使用 `TextComponent`。

## 构建验证
- Fabric 三版本 + Forge 三版本 build 全通过（-x test）。

## 通信器尺寸对齐与交互补强（夜间追加）
- 目标：对齐 `.reference/NyaSamaTelecom` 放置体积（不再占满半砖），并保持移植后通信交互一致。

### 本轮实现
1. 通信方块薄型体积（Shape/Collision）
- 统一采用薄型体积：底座 `1..15 x 0..2 x 1..15` + 上层面板 `2..14 x 2..3 x 2..14`。
- 文件：
  - `fabric-1.20.1/src/main/java/cn/hydcraft/hydronyasama/fabric/TelecomRenderBlock.java`
  - `forge-1.20.1/src/main/java/cn/hydcraft/hydronyasama/forge/TelecomRenderBlock.java`
  - `fabric-1.18.2/src/main/java/cn/hydcraft/hydronyasama/fabric/TelecomInteractiveBlock.java`
  - `fabric-1.16.5/src/main/java/cn/hydcraft/hydronyasama/fabric/TelecomInteractiveBlock.java`（新增）
  - `forge-1.18.2/src/main/java/cn/hydcraft/hydronyasama/forge/TelecomInteractiveBlock.java`（新增）
  - `forge-1.16.5/src/main/java/cn/hydcraft/hydronyasama/forge/TelecomInteractiveBlock.java`（新增）

2. 1.16.5/1.18.2 通信方块注册切换
- 由普通 `Block` 切换为通信专用交互方块，确保“空手右击反馈 + 薄型碰撞”生效。
- 文件：
  - `fabric-1.16.5/src/main/java/cn/hydcraft/hydronyasama/fabric/content/FabricContentRegistrar.java`
  - `fabric-1.18.2/src/main/java/cn/hydcraft/hydronyasama/fabric/content/FabricContentRegistrar.java`
  - `forge-1.16.5/src/main/java/cn/hydcraft/hydronyasama/forge/ForgeContentRegistry.java`
  - `forge-1.18.2/src/main/java/cn/hydcraft/hydronyasama/forge/ForgeContentRegistry.java`

3. common 通信模型外观统一改薄
- 批量调整 telecom 17 个 block model 为薄设备外观（不再半砖体）。
- 路径：`common/src/main/resources/assets/hydronyasama/models/block/`
- 覆盖：
  - `signal_box*`、`tri_state_signal_box`
  - `nspga_*`、`nsasm_box`
  - `timer`、`delayer`、`rs_latch`
...
- 说明：并行构建曾触发 `:common:processResources` 目录并发清理冲突，已改串行验证。

## 代码格式化工具接入（Spotless）
- 按仓库需求新增统一格式化任务，根脚本接入 Spotless + Google Java Format。
- 修改：`build.gradle.kts`
  - 新增插件：`com.diffplug.spotless`（6.25.0）
  - 子模块统一规则：`googleJavaFormat(1.17.0)`、移除未使用 import、尾空格清理、文件结尾换行
  - 新增根任务：
    - `spotlessApplyAll`
    - `spotlessCheckAll`

### 格式化验证
- `./gradlew.bat spotlessApplyAll --no-parallel` 通过
- `./gradlew.bat spotlessCheckAll --no-parallel` 通过
...
- 需求来源：
  ...
  2) 顶部出现“细长方形+叉线”贴图；
  3) 通信器体积误用 NSPGA 薄模型；
  4) 通信器种类需与 `.reference/NyaSamaTelecom` 再核对；
  5) 指示灯/白色按钮贴图需恢复参考风格。

### 实施内容
1. NSPGA 中文名称修正
- 文件：`common/src/main/resources/assets/hydronyasama/lang/zh_cn.json`
- 将 5 个 NSPGA 条目改为中文名称（可编程逻辑门阵列/复杂可编程逻辑门阵列）。

2. 通信器体积按 reference 分档
- 依据参考 `setInfo`：
  - NSPGA 系：`0.875 x 0.125 x 0.875`（薄）
  - SignalBox/Timer/Delayer/RS/NSASM：`0.75 x 0.5 x 0.75`（厚）
- 修改文件：
  - `fabric-1.20.1/.../TelecomRenderBlock.java`
  - `forge-1.20.1/.../TelecomRenderBlock.java`
  - `fabric-1.16.5/.../TelecomInteractiveBlock.java`
  - `fabric-1.18.2/.../TelecomInteractiveBlock.java`
  - `forge-1.16.5/.../TelecomInteractiveBlock.java`
  - `forge-1.18.2/.../TelecomInteractiveBlock.java`
- 处理方式：按 `blockPath.startsWith("nspga_")` 选择薄/厚碰撞箱与轮廓。

3. 移除错误的顶部十字调试层（右击变绿来源）
- 文件：
  - `fabric-1.20.1/.../TelecomRenderBlockEntityRenderer.java`
  - `forge-1.20.1/.../TelecomRenderBlockEntityRenderer.java`
- 说明：去掉线框状态叠加，避免与原版外观冲突。

4. 通信模型重做（薄/厚分离，移除顶面额外细条）
- 文件：`common/src/main/resources/assets/hydronyasama/models/block/` 下 17 个 telecom 模型重写。
- NSPGA：仅保留薄底板，去除顶面额外细条。
- 通信器：改为 12x12 底座、半砖高度（更粗、占地更小）。

5. 贴图恢复为 reference 风格（不手绘）
- 从 `.reference/NyaSamaTelecom` 同步通信器基础贴图到 common：
  - `signal_box_base/gt/redstone/wireless`
  - `tri_state_signal_box_base`
  - `nsasm_box_base`
  - `timer_base`
  - `delayer_base`
  - `rs_latch_base`
  - 指示灯调色贴图：`signal_box_r/y/g/w/b/none/p`
- 并将对应模型 top/particle 映射到上述贴图。

6. 补齐缺失通信装饰块
- 新增 `nst_nsdn`（reference 有、此前缺失）：
  - `common/src/main/resources/assets/hydronyasama/blockstates/nst_nsdn.json`
  - `common/src/main/resources/assets/hydronyasama/models/block/nst_nsdn.json`
  - `common/src/main/resources/assets/hydronyasama/models/item/nst_nsdn.json`
  - `common/src/main/java/cn/hydcraft/hydronyasama/content/LegacyContentIds.java`（加入 `CORE_BLOCK_IDS`）
...
### 构建验证
- 执行：
  - `./gradlew.bat :fabric-1.16.5:build :fabric-1.18.2:build :fabric-1.20.1:build :forge-1.16.5:build :forge-1.18.2:build :forge-1.20.1:build -x test --no-parallel`
- 结果：6 目标全部通过。

### 格式化
- 执行：`./gradlew.bat spotlessApplyAll --no-parallel`
- 全仓格式规则已应用并通过构建链路。

## 通信器动态指示灯与状态同步（本轮继续）
- 目标：恢复通信器“顶部三灯+白按钮”状态可视反馈，并避免仅单机可见。

### 实施
1. 1.20.1 通信状态从服务端同步到客户端 BE
- 新增 `TelecomRenderBlockEntity` 面板状态字段：`input/enabled/output`。
- 服务端每 tick 按 endpoint 读取 `TelecomCommService.snapshot(...)` 并写入 BE。
- 通过 `sendBlockUpdated + getUpdatePacket/getUpdateTag` 同步到客户端渲染。
- 文件：
  - `fabric-1.20.1/src/main/java/cn/hydcraft/hydronyasama/fabric/TelecomRenderBlockEntity.java`
  - `forge-1.20.1/src/main/java/cn/hydcraft/hydronyasama/forge/TelecomRenderBlockEntity.java`

2. 1.20.1 通信方块接入 BE ticker
- `TelecomRenderBlock#getTicker(...)` 仅服务端启用，驱动上述状态写入。
- 文件：
  - `fabric-1.20.1/src/main/java/cn/hydcraft/hydronyasama/fabric/TelecomRenderBlock.java`
  - `forge-1.20.1/src/main/java/cn/hydcraft/hydronyasama/forge/TelecomRenderBlock.java`

3. 1.20.1 渲染器恢复动态面板（替代此前临时 no-op）
- 顶部三灯：
  - 左灯：`input`（黄/灭）
  - 中灯：`enabled`（绿/红）
  - 右灯：`output`（蓝/灭）
- 通信器类方块补白按钮位（白/绿）。
- 贴图使用 common 现有调色贴图：`signal_box_r/y/g/b/w/none`。
- 文件：
...
### 构建验证
- `./gradlew.bat :fabric-1.20.1:build -x test --no-parallel` 通过。
- `./gradlew.bat :forge-1.20.1:build -x test --no-parallel` 通过。
- 格式化：
  - `./gradlew.bat :fabric-1.20.1:spotlessApply --no-parallel`
  - `./gradlew.bat :forge-1.20.1:spotlessApply --no-parallel`
