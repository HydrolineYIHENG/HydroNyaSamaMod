...
val architecturyRelocationBase = "cn.hydcraft.hydronyasama.shaded.architectury"
val sqliteRelocationBase = "cn.hydcraft.hydronyasama.shaded.sqlite"
            ...
            java.util.zip.ZipOutputStream(java.io.BufferedOutputStream(java.io.FileOutputStream(tempFile))).use { out ->
                ...
                while (entries.hasMoreElements()) {
                    ...
                    if (name == "architectury.common.json") {
                        ...
                        val text = zip.getInputStream(entry).bufferedReader(Charsets.UTF_8).readText()
                        val cleaned = text.replace("org.sqlite.JDBC", "cn.hydcraft.hydronyasama.shaded.sqlite.JDBC")
                            .lines()
                            .map { it.trim() }
                        ...
                        val cleaned = text.replace(
                            ...
                            "cn.hydcraft.hydronyasama.shaded.sqlite.nativeimage.SqliteJdbcFeature"
                        )
                        out.write(cleaned.toByteArray(Charsets.UTF_8))
                    } else {
