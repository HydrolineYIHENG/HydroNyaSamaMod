...
Provider 会在 `config/hydroline/HydroNyaSama.json` 自动生成配置（Fabric & Forge 均使用 `FabricLoader.getConfigDir()`/`FMLPaths.CONFIGDIR`）。结构：
...
```java
Path configDir = Paths.get("config", "hydroline", "HydroNyaSama.json");
...
1. **公共模块**
   ...
   - `NettyConfig`：负责读取/写入 `config/hydroline/HydroNyaSama.json`，支持自动创建默认值；
...
- **Q：同一 Mohist 实例中已经占用 8888 端口怎么办？**
  A：修改 `config/hydroline/HydroNyaSama.json` 的 `listenPort`，重启服务器即可。Bukkit 插件读取同文件后会自动使用新端口。
