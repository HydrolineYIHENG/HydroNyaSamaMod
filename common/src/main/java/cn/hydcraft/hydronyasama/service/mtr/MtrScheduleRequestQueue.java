...
/**
 * Serializes heavy schedule requests so we only run one at a time and keep a fixed delay between
 * them.
 */
final class MtrScheduleRequestQueue {
  private static final long RATE_LIMIT_INTERVAL_MS =
      Long.getLong("beacon.scheduleRateLimitMs", 400L);
  private static final long REQUEST_TIMEOUT_MS =
      Long.getLong("beacon.scheduleRequestTimeoutMs", 30_000L);
  private static final int MAX_PENDING_REQUESTS = 64;
  private static final BlockingQueue<Runnable> WORK_QUEUE =
      new LinkedBlockingQueue<>(MAX_PENDING_REQUESTS);
  ...
  public static <T> T submit(String label, Callable<T> task)
    ...
    try {
      ...
      BeaconProviderMod.LOGGER.warn(
          "Schedule queue full ({}). Pending={}", label, WORK_QUEUE.size());
