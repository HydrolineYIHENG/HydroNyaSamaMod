...
import cn.hydcraft.hydronyasama.transport.TransportKind;
...
public final class GatewayServer implements AutoCloseable {
  ...
  public synchronized void start() {
    ...
    if (config.listenPort() <= 0) {
      BeaconProviderMod.LOGGER.info(
          "Beacon Netty gateway disabled (listenPort={})", config.listenPort());
      return;
    }
    ...
    ChannelFuture bound =
        bootstrap.bind(new InetSocketAddress(config.listenAddress(), config.listenPort()));
  ...
  private final class GatewayChannelHandler extends SimpleChannelInboundHandler<ByteBuf> {
            ...
            () -> {
              if (!connection.handshakeComplete()) {
                BeaconProviderMod.LOGGER.warn(
                    "Gateway connection {} did not complete handshake in time",
                    connection.temporaryId());
    ...
    public void userEventTriggered(io.netty.channel.ChannelHandlerContext ctx, Object evt)
      ...
      if (evt instanceof IdleStateEvent) {
        BeaconProviderMod.LOGGER.info(
            "Closing idle gateway connection {}", connection.temporaryId());
  ...
  private void handleHandshake(
      io.netty.channel.ChannelHandlerContext ctx, GatewayConnection connection, JsonObject body) {
    if (connection.handshakeComplete()) {
    ...
    int protocolVersion =
        body.has("protocolVersion")
            ? body.get("protocolVersion").getAsInt()
            : ChannelConstants.PROTOCOL_VERSION;
  ...
  private void handleRequest(GatewayConnection connection, JsonObject body) {
    ...
    TransportContext context =
        new TransportContext(connection.connectionId(), TransportKind.NETTY_GATEWAY, Instant.now());
  ...
  private void sendError(Channel channel, ResultCode code, String message) {
    ...
    channel.writeAndFlush(
        channel
            .alloc()
            .buffer()
            .writeBytes(GatewayCodec.encode(GatewayMessageType.ERROR, null, body)));
  }
