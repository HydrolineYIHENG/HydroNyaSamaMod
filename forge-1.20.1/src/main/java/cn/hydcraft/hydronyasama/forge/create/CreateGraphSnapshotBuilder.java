...
final class CreateGraphSnapshotBuilder {
  ...
  static CreateNetworkSnapshot build(TrackGraph graph, long updatedAt) {
    ...
    CreateNetworkSnapshot.GraphInfo graphInfo =
        new CreateNetworkSnapshot.GraphInfo(graphId, checksum, color, updatedAt);
  ...
  private static void appendEdgePoints(
      TrackGraph graph,
      TrackEdge edge,
      String edgeId,
    ...
    for (TrackEdgePoint point : points) {
      if (point instanceof GlobalStation) {
        ...
        Vec3 pos = resolvePointPosition(graph, edge, position, length);
        String groupPrimary =
            boundary.getGroup(edge.node1) == null ? null : boundary.getGroup(edge.node1).toString();
        String groupSecondary =
            boundary.getGroup(edge.node2) == null ? null : boundary.getGroup(edge.node2).toString();
        boundaries.add(
  ...
  private static void addSegment(
    ...
    segments.add(
        new CreateNetworkSnapshot.EdgeSegmentInfo(
            edgeId + ":" + index, edgeId, start, end, groupId));
  }

  private static Vec3 resolvePointPosition(
      TrackGraph graph, TrackEdge edge, double position, double length) {
    if (length <= 0) {
