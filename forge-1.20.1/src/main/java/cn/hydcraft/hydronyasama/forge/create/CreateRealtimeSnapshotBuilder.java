...
final class CreateRealtimeSnapshotBuilder {
  ...
  static CreateRealtimeSnapshot capture(MinecraftServer server) {
    ...
    List<CreateRealtimeSnapshot.TrainStatus> trains =
        new ArrayList<CreateRealtimeSnapshot.TrainStatus>();
    for (Map.Entry<UUID, Train> entry : data.getTrains().entrySet()) {
      Train train = entry.getValue();
      trains.add(buildTrainStatus(train));
    }
    List<CreateRealtimeSnapshot.GroupStatus> groups =
        new ArrayList<CreateRealtimeSnapshot.GroupStatus>();
  ...
  private static CreateRealtimeSnapshot.TrainStatus buildTrainStatus(Train train) {
    ...
    String graphId =
        train.graph == null || train.graph.id == null ? null : train.graph.id.toString();
    ...
    Integer scheduleEntry =
        train.runtime == null ? null : Integer.valueOf(train.runtime.currentEntry);
    String scheduleState =
        train.runtime == null || train.runtime.state == null ? null : train.runtime.state.name();
    ...
    List<CreateRealtimeSnapshot.DimensionPosition> positions =
        new ArrayList<CreateRealtimeSnapshot.DimensionPosition>();
    for (ResourceKey<Level> dimension : train.getPresentDimensions()) {
      ...
      if (pos != null) {
        positions.add(
            new CreateRealtimeSnapshot.DimensionPosition(
                dimension.location().toString(), pos.getX(), pos.getY(), pos.getZ()));
      }
    }

    List<CreateRealtimeSnapshot.CarriageInfo> carriages =
        new ArrayList<CreateRealtimeSnapshot.CarriageInfo>();
  ...
  private static CreateRealtimeSnapshot.CarriageInfo buildCarriageInfo(
      TrackGraph graph, Carriage carriage) {
    CreateRealtimeSnapshot.TravellingPointInfo leading =
        buildTravellingPointInfo(graph, carriage.getLeadingPoint());
    CreateRealtimeSnapshot.TravellingPointInfo trailing =
        buildTravellingPointInfo(graph, carriage.getTrailingPoint());
    ...
    return new CreateRealtimeSnapshot.CarriageInfo(
        carriage.id, carriage.bogeySpacing, leading, trailing, leadingBogey, trailingBogey);
  }
  ...
  private static CreateRealtimeSnapshot.TravellingPointInfo buildTravellingPointInfo(
      TrackGraph graph, TravellingPoint point) {
    ...
    if (node1 != null
        && node1.getLocation() != null
        && node1.getLocation().getDimension() != null) {
    ...
    if (graph != null && edge != null && edge.node1 != null && edge.node2 != null) {
      String materialId =
          edge.getTrackMaterial() == null ? null : edge.getTrackMaterial().id.toString();
      edgeId =
          CreateEdgeIdFactory.build(
              graph.id.toString(),
              edge.node1.getNetId(),
              edge.node2.getNetId(),
              edge.isTurn(),
  ...
  private static CreateRealtimeSnapshot.GroupStatus buildGroupStatus(SignalEdgeGroup group) {
    ...
    String reservedBoundaryId =
        group.reserved == null || group.reserved.getId() == null
            ? null
            : group.reserved.getId().toString();
